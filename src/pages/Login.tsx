...
const Login = () => {
  ...
  const targetAfterLogin = (roleToUse: "customer" | "vendor") => {
    const requestedPath = (location.state as { from?: string } | null)?.from;
    if (requestedPath && requestedPath !== "/login") return requestedPath;
    return roleToUse === "vendor" ? "/vendor-dashboard" : "/dashboard";
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!hasSupabaseEnv) {
      toast({
        ...
        description: "Set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY to enable login.",
      });
      return;
    }
    ...
    const resolvedRole = user?.user_metadata?.role === "vendor" ? "vendor" : role;
    toast({
      title: "Welcome back",
      description: "You are now logged in.",
    });
    navigate(targetAfterLogin(resolvedRole));
  };
  ...
  const handleGoogle = async () => {
    if (!hasSupabaseEnv) {
      toast({
        title: "Supabase not configured",
        description: "Set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY to enable Google login.",
      });
      return;
    }
        ...
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" variant="hero" size="lg" className="w-full" disabled={submitting}>
              {submitting ? "Logging in..." : role === "vendor" ? "Log In as Vendor" : "Log In as Customer"}
            </Button>
